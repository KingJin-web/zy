...
def login(wid, wm_phone, user_logs):
    step = "登录"
    try:
        url = "https://api.zhumanito.cn/api/login"
        payload = {"wid": wid, "wm_phone": wm_phone}
        headers = {'User-Agent': user_agent, 'Content-Type': "application/json"}
        response = requests.post(url, data=json.dumps(payload), headers=headers)
        response.raise_for_status()
        dljson = response.json()
        if 'data' in dljson and 'token' in dljson['data'] and 'user' in dljson['data'] and 'land' in dljson['data']:
            msg = "登录成功 ✅"
            print(msg)
        ...
        if water >= 20 and sun >= 20:
            ...
            try:
                ...
                if res.get("code") == 200:
                    ...
                    # 浇水失败（如达上限、其他错误）
                    fail_msg = f"❌ 账号{account_idx}：浇水失败：{res.get('msg', '未知错误')}"
                    print(fail_msg)
...
def process_user(user_info, user_index):
    # 解析用户信息: wid#wm_phone
    parts = user_info.split("#")
    if len(parts) == 2:
        wid, wm_phone = parts[0].strip(), parts[1].strip()
    else:
        wid, wm_phone = parts[0].strip(), ""
    
    user_logs = [f"👤 用户{user_index}: {wid} (手机: {wm_phone if wm_phone else '未提供'})"]
    print(f"\n===== 开始处理用户 {user_index} (wid: {wid}, 手机: {wm_phone if wm_phone else '未提供'}) =====")
    login_data = login(wid, wm_phone, user_logs)
    if login_data:
...
if __name__ == "__main__":
    if not users or len(users) == 0:
        ...
        for i, user_info in enumerate(users, 1):
            try:
                user_logs = process_user(user_info, i)
